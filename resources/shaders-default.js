...
void main() {    
    ...
    // 灰度颜色定义（使用亮度转换公式）  
    float color1 = dot(vec3(0.4, 0.6, 0.9), vec3(0.2126, 0.7152, 0.0722)); // 原色亮度值  
    float color2 = dot(vec3(0.2, 0.3, 0.7), vec3(0.2126, 0.7152, 0.0722)); // 原色亮度值  
    float activeColor = 1.0;  // 白色作为高亮  
      
    float gradientFactor = smoothstep(0.0, 1.0, distortedUV.y);    
    float baseColor = mix(color2, color1, gradientFactor);    
    ...
    gl_FragColor = vec4(vec3(finalColor * vignette), alpha);    
}  
`; 